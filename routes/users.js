...
const transporter = nodemailer.createTransport(
    sendgridTransport({auth : {api_key : config.nodemailer_api}}));
...
router.post('/search-users', (req, res, next) => {
  ...
  Users
      ...
      .select("_id email name pic")
      .then(users => { res.json({users : users}); }, err => next(err))
      .catch((err) => next(err));
});
              ...
              Users
                  ...
                  .then((user) => {
                    transporter.sendMail({
                      ...
                      html :
                          "<h2>Welcome to Connect</h2><p>Your account has been created Successfully</p><p><a href='https://connect-in.herokuapp.com/'>click here</a>to login into your account</p>"
                    })
...
router.put('/addtofav', authenticate.verifyUser,
           (req, res, next) => {
               Users
...
router.put('/removefromfav', authenticate.verifyUser,
           (req, res, next) => {
               Users
                   .findByIdAndUpdate(req.user._id,
...
router.put(
    '/follow', authenticate.verifyUser,
    (req, res, next) => {Users.findByIdAndUpdate(
        req.body.followId, {$push : {followers : req.user._id}}, {new : true},
        (err, result) => {
          if (err) {
            return res.status(422).json({error : err});
          }
...
router.put(
    '/unfollow', authenticate.verifyUser,
    (req, res, next) => {Users.findByIdAndUpdate(
        req.body.unfollowId, {$pull : {followers : req.user._id}}, {new : true},
        (err, result) => {
          if (err) {
            return res.status(422).json({error : err});
          }
...
router.put('/updatepic', authenticate.verifyUser,
           (req, res, next) => {Users.findByIdAndUpdate(
               req.user._id, {$set : {pic : req.body.pic}}, {new : true},
               (err, result) => {
                 if (err) {
...
router.post('/reset-password',
            (req, res, next) => {crypto.randomBytes(32, (err, buffer) => {
              if (err) {
                console.log(err);
              }
